...
Logo 区：绿色方块"妙" + 标题"AI导演——妙微的工作台" + 副标"miaowei STUDIO"（2026-07-26 定稿）。
...
- 导航栏图标 = Lucide 风格细线条透明 SVG（内联，`stroke="currentColor"`，16px，激活态自动变色），不用彩色 emoji
...
## 导航栏结构（2026-07-26 按姜Dora框架重构，两组10板块）

- **开发管理**：1 灵感速记（=原灵感速记，功能不变）｜2 项目计划（占位）｜3 Skills 管理（占位）｜4 品牌合作（保持空白）｜5 后台管理（占位）
- **内容运营**：6 爆款视频Lab（=原爆款拆解库，data-mod 仍叫"爆款拆解库"）｜7 选题合集库（=原选题库）｜8 写作室（=原脚本写作室）｜9 第二大脑（新骨架 viewBrain，4 子页签：内容卡片库/访谈问题库/经验方法库/知识库管理，方案见 `第二大脑方案.md`）｜10 制作与发布（=原制作与发布）
- 路由 hash 直达：`#ideas` `#plan` `#skills` `#brand` `#admin` `#hot` `#topics` `#scripts` `#brain` `#prod`
- 撤下导航的旧模块（数据复盘/提示词库/导演参考库/任务看板/商务合作）视图仍在页面里，hash 路由可用；它们的侧栏徽标 span 收在一个 `display:none` 的 div 里（防 JS 取元素报错），后续逐步并入新板块
- 新占位板块点击后显示"规划中"提示页，data-mod 与 hash 映射见 JS 里 HASH2MOD/MOD2HASH
